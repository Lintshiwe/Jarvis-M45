...
if _OS == "Windows":
    _SAFE_ROOTS += [
        ...
        Path(os.environ.get("TMP", str(Path.home() / "AppData" / "Local" / "Temp"))),
        Path("C:/Users"),
    ]
else:
    _SAFE_ROOTS += [
...
# System paths that are NEVER allowed (even under safe roots)
if _OS == "Windows":
    _FORBIDDEN_PREFIXES = [
        "C:\\Windows\\System32\\",
        "C:\\Windows\\SysWOW64\\",
    ...
    _FORBIDDEN_PREFIXES = [
        "/System/", "/Library/System/", "/usr/lib/", "/usr/libexec/",
        "/bin/", "/sbin/", "/private/etc/", "/private/var/db/",
    ]
else:
    _FORBIDDEN_PREFIXES = [
        "/etc/", "/boot/", "/sys/", "/proc/", "/dev/",
        "/root/", "/run/systemd/", "/run/user/0/",
...
def _is_safe_path(target: Path) -> bool:
    """Check if path is under a safe root and NOT in a forbidden system directory."""
    try:
        ...
        if _OS == "Windows":
            ...
            for forbidden in _FORBIDDEN_PREFIXES:
                if resolved_str_lower.startswith(forbidden.lower()):
                    return False
            return any(
                resolved_str_lower.startswith(str(root.resolve()).lower())
