...
def _capture_screen() -> tuple[bytes, str]:
    ...
    # On Linux/Wayland, mss often returns blank images.
    # Try native screenshot tools first, fall back to mss.
    if platform.system() == "Linux":
        png = _capture_screen_linux()
        if png:
...
def _capture_screen_linux() -> bytes | None:
    """Try native Linux screenshot tools (works on both X11 and Wayland)."""

    tools = [
        # gnome-screenshot-tool: GNOME 40+ (X11 + Wayland)
        lambda: _try_screenshot_tool_file(
            ["gnome-screenshot-tool", "-f"], ".png"
        ),
        # grim: wlroots/Sway/Wayland
        lambda: _try_screenshot_tool(["grim", "-"]),
        # gnome-screenshot (legacy): GNOME < 40
        lambda: _try_screenshot_tool_file(
...
def _try_screenshot_tool(cmd: list[str]) -> bytes | None:
    """Try a screenshot tool that writes PNG to stdout."""
...
def _try_screenshot_tool_file(cmd: list[str], ext: str) -> bytes | None:
    """Try a screenshot tool that writes to a temp file."""
    fd, path = tempfile.mkstemp(suffix=ext)
    os.close(fd)
    try:
        result = subprocess.run(
            cmd + [path], capture_output=True, timeout=5
        )
