...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            ...
            # Try real profile first (works on Windows/macOS, may fail on Linux)
            real_profile = _firefox_profile_dir()
            if real_profile:
                try:
                    self._context = await asyncio.wait_for(
                        engine_obj.launch_persistent_context(real_profile, **kwargs),
                        timeout=15
                    )
                    print(f"[Browser] Firefox launched with real profile")
                except Exception:
                    real_profile = None  # fall through

            # Fallback: JARVIS-managed profile
            if real_profile is None or self._context is None:
                print(f"[Browser] Using JARVIS Firefox profile")
    ...
    async def get_text(self, full: bool = False) -> str:
        page = await self._get_page()
        try:
            text = await page.inner_text("body")
            limit = 20_000 if full else 4_000
            return text[:limit]
        except Exception as e:
    ...
    async def extract_content(self, full_page: bool = True) -> str:
        """Extract structured content: title, headings, paragraphs, links, images."""
        page = await self._get_page()
        try:
            ...
            for level in range(1, 7):
                try:
                    els = await page.locator(f"h{level}").all()
                    for el in els:
                        txt = (await el.inner_text()).strip()
                        if txt and len(txt) > 2:
                            headings.append(("h" * level, txt))
                except Exception:
                    pass
            if headings:
                parts.append("## Headings")
                for prefix, txt in headings[:30]:
                    parts.append(f"{'#' * len(prefix)} {txt}")
                parts.append("")
            ...
            try:
                ...
                for i in range(min(count, 50)):
                    try:
                        ...
                        txt = (await el.inner_text()).strip()
                        if href and not href.startswith("#") and href not in seen_urls and txt:
                            seen_urls.add(href)
                        ...
                        continue
                if link_count == 0:
                    parts.append("(no links extracted)")
            except Exception:
                parts.append("(link extraction failed)")
                ...
                for i in range(min(img_count, 20)):
                    try:
                        ...
                        if src and src not in seen_urls:
                            seen_urls.add(src)
                            parts.append(f"- {alt if alt else 'Image'}: {src}")
                            img_extracted += 1
                    except Exception:
                        continue
                if img_extracted == 0:
                    parts.append("(no images extracted)")
            except Exception:
                parts.append("(image extraction failed)")
            ...
            for i in range(min(count, 100)):
                try:
                    ...
                    if href and href not in seen and not href.startswith("#"):
                        seen.add(href)
                        result.append(f"{txt:40s} → {href}")
                except Exception:
                    continue
            if not result:
                return "No links found on the page."
            return f"Links found: {len(result)}\n\n" + "\n".join(result)
        except Exception as e:
            return f"Link extraction error: {e}"

    async def capture_social_media(self, platform: str = "", count: int = 10) -> str:
        """Extract posts/tweets/comments from social media pages."""
        page = await self._get_page()
        url = page.url.lower()
        platform = platform.lower() or "auto"

        try:
            content_parts = [f"# Social Media Extract — {page.url}", ""]
            ...
            if is_twitter or platform in ("twitter", "x"):
                content_parts.append("## Tweets/Posts")
                # Extract tweet text
                for selector in [
                    ...
                    try:
                        ...
                        for i in range(min(el_count, count)):
                            try:
            ...
            elif is_reddit or platform == "reddit":
                content_parts.append("## Reddit Posts/Comments")
                for selector in [
                    'shreddit-post',
            ...
            elif is_instagram or platform == "instagram":
                content_parts.append("## Instagram Content")
                for selector in [
                    'article', 'div._aagw', 'h1', 'h2',
                    ...
                    try:
                        ...
                        for i in range(min(el_count, count)):
                            try:
                                txt = (await els.nth(i).inner_text()).strip()
                                if txt and len(txt) > 3 and not txt.lower().startswith("http"):
                                    content_parts.append(f"- {txt[:300]}")
                            except Exception:
                                pass
                        if len(content_parts) > 1:
                            break
            ...
            elif is_facebook or platform == "facebook":
                content_parts.append("## Facebook Content")
                for selector in [
                    'div[role="article"]',
            ...
            elif is_linkedin or platform == "linkedin":
                content_parts.append("## LinkedIn Content")
                for selector in [
                ...
                content_parts.append("## Page Content (generic)")
                text = await page.inner_text("body")
                text = re.sub(r"\n{3,}", "\n\n", text)
                text = re.sub(r" {2,}", " ", text)
                content_parts.append(text[:5_000])

            if len(content_parts) <= 1:
                content_parts.append("No structured content could be extracted from this page.")

            return "\n".join(content_parts)

        except Exception as e:
            return f"Social media extraction error: {e}"
...
def browser_control(
    ...
    try:
        if action == "go_to":
            ...
            full = str(params.get("full", "")).lower() in ("true", "yes", "1")
            result = sess.run(sess.get_text(full=full))
        elif action == "extract_content":
            full_page = str(params.get("full_page", "true")).lower() != "false"
            result = sess.run(sess.extract_content(full_page=full_page))
        elif action == "extract_links":
            result = sess.run(sess.extract_links())
        elif action == "capture_social_media":
            result = sess.run(sess.capture_social_media(
                params.get("platform", ""),
