...
def close_app(app_name: str = ""):
    """Close the focused app, or close a specific app by name."""
    if app_name:
        _close_app_by_name(app_name)
        return
    if _OS == "Darwin": pyautogui.hotkey("command", "q")
    else:               pyautogui.hotkey("alt", "f4")


def _close_app_by_name(app_name: str):
    """Force-close a specific application by name."""
    name = app_name.lower().strip()
    if _OS == "Linux":
        # Try killall first (fast, no /proc scan)
        try:
            result = subprocess.run(
                ["killall", "-q", name],
        ...
        # Focus the window by name, then Alt+F4
        try:
            # Use wmctrl to focus, then close
            subprocess.run(["wmctrl", "-a", name], capture_output=True, timeout=3)
            time.sleep(0.3)
        ...
        # Fallback: use Super key to open overview, type app name, then Alt+F4
        try:
...
def list_windows_linux() -> str:
    """List open application windows on Linux using wmctrl or xdotool."""
    if _OS == "Linux":
        try:
            result = subprocess.run(
        ...
        try:
            result = subprocess.run(
                ["xdotool", "search", "--name", ""], capture_output=True, text=True, timeout=5
            )
            if result.returncode == 0:
                ...
                for wid in window_ids:
                    try:
                        ...
                        if name.stdout.strip():
                            names.append(f"{wid}  {name.stdout.strip()}")
                    except Exception:
                        pass
...
def navigate_settings(section: str = ""):
    """Navigate to a section within system settings using keyboard search."""
    # First ensure settings is open and focused
    open_system_settings()
    time.sleep(1.5)
    # Use Ctrl+F or just type to search within settings
    if _OS == "Linux":
        find_on_page()  # Ctrl+F in most Linux settings apps
    elif _OS == "Darwin":
...
ACTION_MAP: dict[str, callable] = {
    ...
    "system_monitor":      open_task_manager,
...
def _extract_app_name(description: str) -> str:
    """Extract app name from a natural language phrase like 'Exit Spotify' or 'Close chrome'."""
    import re
    
    desc = description.lower().strip()
    # Remove common command verbs
    verbs = [
        "exit", "close", "quit", "kill", "terminate", "stop", "shut down",
        "shutdown", "end", "force quit", "force close",
    ]
    for verb in sorted(verbs, key=len, reverse=True):
        desc = desc.replace(verb, "")
    # Remove filler words
    for word in ["the", "application", "app", "program", "window", "process", "please", "can you", "could you"]:
...
def _detect_action(description: str) -> dict:
    ...
    available = ", ".join(sorted(ACTION_MAP.keys())) + \
                ", volume_set, type_text, press_key, reload_n, navigate, list_windows"
...
def computer_settings(
    ...
    if action in ("close_app", "close", "quit", "exit_app", "kill", "terminate", "exit", "stop_app", "shut_down"):
        app_name = str(value or "").strip()
        # If no value, try to extract app name from description using AI
        if not app_name and description:
            app_name = _extract_app_name(description)
        if not app_name:
    ...
    if action in ("navigate", "navigate_to", "goto"):
        section = str(value or description or "").strip()
        try:
            navigate_settings(section)
            return f"Navigated to: {section}" if section else "Navigated in settings."
        except Exception as e:
            return f"Navigation failed: {e}"

    if action in ("list_windows", "list_apps", "list_applications", "list"):
        try:
            result = list_windows_linux()
            if result:
                return f"Open windows:\n{result}"
            return "No windows found or listing not supported on this OS."
        except Exception as e:
    ...
    if not func:
        # Fuzzy fallback: try matching action name within known actions
        all_actions = list(ACTION_MAP.keys())
        candidates = [a for a in all_actions if action in a or a in action]
        if candidates:
            func = ACTION_MAP.get(candidates[0])
        if not func:
            return f"Unknown action: '{raw_action}'. Available: {', '.join(sorted(all_actions))}"
