...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are JARVIS M45, an AI assistant. "
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            "Controls the computer. Use for ANY single system command like: "
            "volume up/down/mute, brightness up/down, closing/killing/terminating/quitting/stopping/exiting apps by name, "
            "maximize/minimize/fullscreen/snap left/snap right current window, "
            "lock screen, dark mode toggle, WiFi on/off, restart/shutdown, "
            "scrolling up/down/top/bottom, tab management (next/prev/close/new), zoom in/out/reset, "
            "copy/paste/cut/undo/redo/select all/save, screenshot, "
            "pause/play video, type text, press keys, navigate settings, open task manager, "
            "file explorer, refresh/reload page, go back/forward, find on page, enter/escape key. "
            "For closing apps: use action='close_app' and put the app name in the 'value' field (e.g. value='spotify'). "
            "The user may say 'Exit Spotify', 'Close Spotify', 'Kill Spotify', 'Terminate Spotify', "
            "'Stop Spotify', 'Shut down Spotify' — all mean the same: close_app with value='spotify'. "
            "Extract the app name from the user's phrase and put it in 'value'."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action":      {"type": "STRING", "description": (
                    "Action: volume_up | volume_down | mute | unmute | brightness_up | brightness_down | "
                    "close_app (close/quit/kill/terminate/exit/stop/shut_down an app by name in 'value') | "
                    "maximize | minimize | full_screen | snap_left | snap_right | "
                    "lock_screen | dark_mode | toggle_wifi | restart | shutdown | "
                    "sleep_display | screenshot | pause_video | play_pause | "
                    "task_manager | file_explorer | open_settings | show_desktop | "
                    "scroll_up | scroll_down | scroll_top | scroll_bottom | page_up | page_down | "
                    "next_tab | prev_tab | close_tab | new_tab | zoom_in | zoom_out | zoom_reset | "
                    "copy | paste | cut | undo | redo | select_all | save | "
                    "refresh_page | go_back | go_forward | find_on_page | "
                    "switch_window | focus_search | open_run | enter | escape | "
                    "type_text | press_key | volume_set"
                )},
                "description": {"type": "STRING", "description": "Fallback: natural description of what to do (only used if action is empty)"},
                "value":       {"type": "STRING", "description": (
                    "For close_app: the app name to close (e.g. 'spotify', 'chrome', 'firefox'). "
                    "For volume_set: volume level 0-100. For type_text: the text to type. "
                    "For press_key: the key name. For find_on_page: search term. "
                    "For restart/shutdown: put 'yes' here if the user confirmed."
                )},
                "confirmed":   {"type": "STRING", "description": "Set to 'yes' to confirm dangerous actions (restart, shutdown)"},
            },
            "required": []
        }
    },
    ...
    },
    {
        "name": "system",
        "description": (
            "Executes system shell commands on the computer. "
            "Use this to: run bash commands, check system status, diagnose issues, "
            "install/remove packages, manage services, view logs, check disk/memory/network/CPU, "
            "list processes, manage files at system level, run any terminal command. "
            "Has built-in diagnostics for: disk, memory, cpu, network, processes, system, "
            "services, logs, errors, users, ports, drivers, packages, updates, temperature, "
            "battery, firewall, ssh, docker, git. "
            "Use action='diagnose' with target='<category>' for quick system checks. "
            "Use action='fix' with target='<category>' for system maintenance. "
            "Use command='<shell command>' to run arbitrary safe commands."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "command":  {"type": "STRING", "description": "The shell command to execute (e.g. 'ls -la', 'df -h', 'systemctl status nginx')"},
                "action":   {"type": "STRING", "description": "Shorthand: 'diagnose', 'check', 'fix', 'info', 'status'"},
                "target":   {"type": "STRING", "description": "What to check: disk, memory, cpu, network, processes, system, services, logs, errors, users, ports, drivers, packages, updates, temperature, battery, firewall, ssh, docker, git"},
                "analyze":  {"type": "BOOLEAN", "description": "Use AI to analyze and summarize the output (default: true)"},
...
class JarvisLive:
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        parts = [sys_prompt]  # personality FIRST so it takes priority
